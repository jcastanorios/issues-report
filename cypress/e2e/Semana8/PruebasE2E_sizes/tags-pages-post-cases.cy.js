...
describe('Testing Ghost', () => {
    ...
    it('Crear y validar nuevo tag', () => {
        ...
        visitGhost("#/tags")
        cy.wait(2000)
        ScreenshotPage.takeScreenshot(nombreEscenario, `Tags`)
        cy.get('a.ember-view.gh-btn.gh-btn-primary').click()  //click en New tag
        cy.wait(1000)
        ScreenshotPage.takeScreenshot(nombreEscenario, `NewTags`)
        ...
        cy.get('a.ember-view.gh-secondary-action.gh-nav-new-post')
            .should('exist')
            .click({force: true}) //click en + post
        cy.wait(1000)
        ...
        cy.wait(1000)
        ScreenshotPage.takeScreenshot(nombreEscenario, `SettingsPost`)
        ...
        if(Constantes.VERSION_GHOST==5){
            cy.get('button.gh-btn.gh-btn-editor.darkgrey.gh-publish-trigger')
                ...
                .click()  //botón publish
                cy.wait(1000)
                ScreenshotPage.takeScreenshot(nombreEscenario, `PublishButton`)
            cy.get('button.gh-btn.gh-btn-black.gh-btn-large')
                .should('exist')
                .click() //botón "continue"
                cy.wait(1000)
                ScreenshotPage.takeScreenshot(nombreEscenario, `ContinuePublishButton`)
            cy.get('button.gh-btn.gh-btn-large.gh-btn-pulse.ember-view')
                .should('exist')
                .click()  //click en publicar de una vez
                cy.wait(1000)
                ScreenshotPage.takeScreenshot(nombreEscenario, `PublishRightNowButton`)
        }
        ...
        if(Constantes.VERSION_GHOST==5){
            ...
            cy.wait(1000)
            ScreenshotPage.takeScreenshot(nombreEscenario, `BackToPost`) 
        }
        else if(Constantes.VERSION_GHOST==4){
            visitGhost("#/posts");
            cy.wait(1000)
    ...
    it('Editar tag y validar', () =>{
        ...
        cy.get('section.gh-canvas > section.view-container.content-list > ol > li')
            ...
            .contains(tituloTag)
            .click({force:true}) // encontrar el tag
        cy.wait(1000)
        ScreenshotPage.takeScreenshot(nombreEscenario, `NewTags`) 
        cy.get('input#tag-name.gh-input').type(tituloTag2) //nombre
        cy.get('button.gh-btn.gh-btn-primary.gh-btn-icon.ember-view').click() //save
        //cy.get('section > div > ul:nth-child(2) > li:nth-child(3)').click({force: true}) //click en Tag
        //Clic en mas
        cy.get('div.gh-mobile-nav-bar-more').click()
        visitGhost("#/tags")//Clic en la herramienta
        cy.wait(2000)
        cy.get('section.gh-canvas > section.view-container.content-list > ol > li')
             .find('h3.gh-tag-list-name')
             .contains(tituloTag2)
    ...
    it('Crear, borrar tag y validar', () =>{
        let tituloTag2=faker.commerce.isbn();
        ...
        cy.get('button.gh-btn.gh-btn-primary.gh-btn-icon.ember-view').click() //save
        //visitGhost("#/tags")//Clic en la herramienta cy.get('section > div > ul:nth-child(2) > li:nth-child(3)').click() //click en Tag
        //cy.wait(1000)
        //cy.get('h3.gh-tag-list-name').contains(tituloTag2).click({force: true}); // Hacer clic en el TAG a editar
        ...
        //ScreenshotPage.takeScreenshot(nombreEscenario, `DeleteTags`)
        cy.get('button.gh-btn.gh-btn-red.gh-btn-icon').click()  // borrar tag
        cy.wait(1000)  // esperar
        ScreenshotPage.takeScreenshot(nombreEscenario, `ConfirmDeleteTags`)
        cy.get('button.gh-btn.gh-btn-red.gh-btn-icon.ember-view').click() //confirmar que borra
        cy.contains('h3.gh-tag-list-name',tituloTag2).should('not.exist');        
        visitGhost("#/posts/")//cy.get('a[href="#/posts/"]').first().click() // clic en post en la barra lateral de navegacion
        cy.get('section.view-actions > div > div:nth-child(4) > div:nth-child(1)').click({force:true})  //selección de All tag
        cy.wait(1000)  // esperar
        ScreenshotPage.takeScreenshot(nombreEscenario, `SelectAllTags`)
        if(Constantes.VERSION_GHOST==5){
            cy.get('body > div:nth-child(1) > div > ul.ember-power-select-options').should('not.exist',tituloTag2)  //seleccionar de la lista de all tag
        }
        else if(Constantes.VERSION_GHOST==4){
            cy.get('body > div:nth-child(1) > div > ul.ember-power-select-options').should('not.exist', tituloTag2)  //seleccionar de la lista de all tag                
        }
    ...
    it('Unpublish un post', () =>{
        ...
        visitGhost("#/posts/")//cy.get('section > div > ul:nth-child(2) > li:nth-child(1)').click() //click en post
        cy.wait(2000)
        ScreenshotPage.takeScreenshot(nombreEscenario, `Posts`)
        cy.get('a.ember-view.gh-btn.gh-btn-primary').click()  //click en New Post
        cy.wait(1000)
        ScreenshotPage.takeScreenshot(nombreEscenario, `NewPosts`)
        const tituloPost = faker.commerce.productName();
        cy.get('textarea.gh-editor-title.ember-text-area.gh-input.ember-view').type(tituloPost) //agregar título
        const textareaPost = faker.lorem.paragraph(3);
        cy.get('div.koenig-editor__editor.__mobiledoc-editor.__has-no-content').type(textareaPost)  //agregar parrafo a post
        ...
        if(Constantes.VERSION_GHOST==5){        
            ...
            cy.wait(1000)
            ScreenshotPage.takeScreenshot(nombreEscenario, `UnpublishButton`)
            cy.get('button.gh-revert-to-draft').click() // unpublish an revert to private draft
            cy.wait(1000)
            ScreenshotPage.takeScreenshot(nombreEscenario, `RevertToDraftPost`)
        ...
        visitGhost("#/posts/?type=draft")//cy.get('a[href="#/posts/?type=draft"]').click()  // click en Drafts
        cy.wait(1000)
        ScreenshotPage.takeScreenshot(nombreEscenario, `Drafts`)  
        if(Constantes.VERSION_GHOST==5){
