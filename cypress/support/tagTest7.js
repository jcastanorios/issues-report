...
class TagTest{
    ...
    CreateNewTag(tituloTag, slug, descript){
    ...
    CreateEmptyNameTag(tituloTag, slug, descript){
        //cy.get('input#tag-name.gh-input').type(tituloTag) //nombre
        cy.get('input#tag-slug.gh-input').type(slug) // slug
        cy.get('textarea#tag-description.gh-input.gh-tag-details-textarea').type(descript) //description
    }
    ...
    errorSave() {
        return cy.get('button.gh-btn.gh-btn-primary.gh-btn-icon.gh-btn-red.ember-view').invoke("text").then((text) => text.trim());
      }
